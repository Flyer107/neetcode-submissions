class Solution {
    ...
    private void dfs(Map<String, List<String>> adj, Set<String> visited, String email, List<String> emailCluster) {
        ...
        // STEP 1: GRAPH BUILDING -> O(N * K) Time
        // The outer loop runs N times (for each account).
        // The inner loop runs up to K times (for each email).
        // Hash map insertions take O(1) time.
        // Total time for this block: N * K operations.

        // STEP 2: GRAPH TRAVERSAL -> O(N * K) Time
        // The 'for' loop combined with the 'dfs()' method visits 
        // every single email and every single edge exactly ONCE. 
        // It does not revisit nodes because of the 'visited' set.
        // Total time to traverse the whole graph: N * K operations

        // STEP 3: SORTING -> O(N*K log(N*K)) Time
        // This is the bottleneck! In the worst-case scenario, 
        // every single email belongs to the same person.
        // That means 'emailCluster' contains all N * K emails.
        // Sorting an array of size M takes M log M time.
        // So sorting here takes (N*K) * log(N*K) time.
    /*
    N = total number of acounts
    K =  max number of emails in a singel acccount

    When you add the chunks together, the equation is:Total Time = $O(N \times K)$ [Building] + $O(N \times K)$ [Traversal] + $O(N \times K \log(N \times K))$ [Sorting]Following the rule of "Keep Only the Dominant Term," the sorting step is the most expensive operation. Therefore, the Final Time Complexity is $O(N \times K \log(N \times K))$.    
    */
